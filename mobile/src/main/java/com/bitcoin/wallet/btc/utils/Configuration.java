...
public class Configuration {
    ...
    public static final String PREFS_KEY_NOTIFY = "notify_coin";
    ...
    //private static final Logger log = LoggerFactory.getLogger(Configuration.class);todo log
    ...
    public boolean getNofity() {
        return prefs.getBoolean(PREFS_KEY_NOTIFY, true);
    }
    ...
    public void updateLastVersionCode(final long currentVersionCode) {
        prefs.edit().putLong(PREFS_KEY_LAST_VERSION, currentVersionCode).apply();

        /*if (currentVersionCode > lastVersionCode) todo log
    ...
    public void touchLastUsed() {
        ...
        //log.info("just being used - last used {} minutes ago", (now - prefsLastUsed) / DateUtils.MINUTE_IN_MILLIS); todo log
    }
    ...
    public ExchangeRate getCachedExchangeRate() {
        if (prefs.contains(PREFS_KEY_CACHED_EXCHANGE_CURRENCY) && prefs.contains(PREFS_KEY_CACHED_EXCHANGE_RATE_COIN)
            ...
            return new ExchangeRate(new org.bitcoinj.utils.ExchangeRate(cachedExchangeRateCoin, cachedExchangeRateFiat),
                    cachedExchangeCurrency != null ? cachedExchangeCurrency : "USD");
        } else {
            return null;
        }
    }
    ...
    public void setCachedExchangeRate(final ExchangeRate cachedExchangeRate) {
        ...
        edit.putLong(PREFS_KEY_CACHED_EXCHANGE_RATE_COIN, cachedExchangeRate.getRate().coin.value);
        edit.putLong(PREFS_KEY_CACHED_EXCHANGE_RATE_FIAT, cachedExchangeRate.getRate().fiat.value);
        edit.apply();
    }
